...
<%
	...
	*/
	...
	/*---------- 2) 테이블 출력 -----------*/
	/*
	select 번호, 사원번호, 이름, 급여, 전체급여평균, 전체급여합계, 전체사원수 
	from
		(select rownum 번호, employee_id 사원번호, last_name 이름, salary 급여,
	...
	*/
	...
	String sql = "	select 번호, 사원번호, 이름, 급여, 전체급여평균, 전체급여합계, 전체사원수 from (select rownum 번호, employee_id 사원번호, last_name 이름, salary 급여, round(avg(salary) over()) 전체급여평균, sum(salary) over() 전체급여합계,count(*) over() 전체사원수 from employees) where 번호 between ? and ?";
...
<h2></h2>
	...
	<!---------------- 페이징--------------------->
		...
		if(minPage > 1) {
		%>
				<a href="./windowsFuntionEmpList.jsp?currentPage=<%=minPage-pagePerPage%>">이전</a>	
		<%
			}
		...
		%>
				<a href="./windowsFuntionEmpList.jsp?currentPage=<%=i%>"><%=i%></a>&nbsp;	
		<%	
			}
		}
